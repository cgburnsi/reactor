...
class GeometryData:
    def __init__(self, lmax=81, nwpts=0):
        ''' Class to Handle Nozzle Geometry Calculations. '''
        ...
        self.nwpts  = nwpts             # [-] (int) Specifies the number of points in the nozzle-wall
    ...
    @property
    def inlet_angle_radians(self) -> float: return self.angi * np.pi / 180.0
...
class ControlData:
    def __init__(self):
        ...
        self.dt         = 0.0
        self.n          = 0
        self.n1         = 1         # [?] (int) Time Loop Integration Loop Parameter
        self.n3         = 2         # [-] (int) Time Loop Integration Loop Parameter
        self.nasm       = 0
        self.ivel       = 0
        self.ichar      = 0
...
def onedim(control, geometry, gcb, bcc, solution):
    ...
    for l in range(control.lmax):                                       # Main loop over axial stations
        if l == 0 and (control.n1d == -1 or control.n1d > 2): continue  # Skip first point for supersonic inlet cases
        x = geometry.xi + geometry.dx * l                               # Calculate the step size in x axis (axial direction)
        
        # Determine Mach number at this station
        if control.n1d < 0: pass                                        # Use subsonic-to-supersonic transition logic
...
def masflo(ISURF, control, geometry, gcb, bcc, solution):
    # Helper Calculations  LMAX, RT, YW, LT, U, V, RO
    
    ldum = control.lmax-1
    
    # Calculate the mass flow and thrust for the 1-D Initial Data Surface
    AREAI    = np.pi * geometry.yw[0]**2
    AREAT    = np.pi * geometry.rt**2 
    #a_throat    = (np.pi * self.yw[self.l_t]**2)
    AREAE    = np.pi * geometry.yw[ldum]**2
    VMI      = np.sqrt(solution.u[0, 0, 0]**2 + solution.v[0, 0, 0]**2)
    VMT      = np.sqrt(solution.u[geometry.lt, 0, 0]**2 + solution.v[geometry.lt, 0, 0]**2)
    VME      = np.sqrt(solution.u[ldum, 0, 0]**2 + solution.v[ldum, 0, 0]**2)
    MASSI    = solution.ro[0, 0, 0] * VMI * AREAI
    MASST    = solution.ro[geometry.lt, 0, 0] * VMT * AREAT
    MASSE    = solution.ro[ldum, 0, 0] * VME * AREAE
    #mass_t      = self.RO[0, 0, self.l_t] * vm_t * a_throat 
    #mass_e      = self.RO[0, 0, ldum] * vm_e * a_exit 
    THRUST   = solution.ro[ldum, 0, 0] * solution.u[ldum, 0, 0]**2 * AREAE
    
    print(f'VME = {VME:<3.2f}, MASSI = {MASSI:<3.4f}, MASST = {MASST:<3.4f}, MASSE = {MASSE:<3.4f}, THRUST = {THRUST:<3.2f}')
    print(f'Expansion Ratio = {AREAE/AREAT:<3.2f} [-], vel_exit = {VME:<3.2f} [m/s], mdot = {cv.convert(MASSI, "kg/s", "lb/s"):<3.5f} [lb/s], thrust = {cv.convert(THRUST, "N", "lbf"):<3.2f} [lbf]')
    return MASSI, THRUST
...
def plots(control, geometry, gcb, bcc, solution):
    #print(solution.ro)
...
def create_test_data():
    ...
    # Create control data
    control         = ControlData()
    control.n1d     = 1                 # [-] (int) Specifies the type of initial surface (1 = subsonic-sonic-supersonic)
    control.lmax    = 21                # [-] (int) Number of points in axial direction
    control.mmax    = 8                 # [-] (int) Number of points in radial direction
    ...
    geometry        = GeometryData(control.lmax, control.mmax)
...
def main():
    ...
    massi, thrust = masflo(0, control, geometry, gcb, bcc, solution)
    
    plots(control, geometry, gcb, bcc, solution)
    
    # Main Loop
    dqm = 0.0; ns=0; nconv=0; nc=0; ldum=1; npc=0
    dxr=1.0/control.dx; dyr = 1.0/geometry.dy
    dxrs = dxr * dxr; #dyrs = dyr * dyr
