...
def onedim(control, geometry, gcb, bcc, solution):
    ...
    def newton_raphson_mach():
        MN3 = 0.01
        for _ in range(20):  # ITER = 1,20
            # --- Compute ABM and ABMC exactly as in Fortran ---
            ABM = acoef + bcoef * MN3**2
            ABMC = ABM**ccoef
    
            # f(MN3)  = ABMC/MN3 - ARATIO
            FM = ABMC / MN3 - aratio
    
            # f'(MN3) = d/dMN3 [ ABM^CCOEF / MN3 ]
            #        = ABMC * ( 2*BCOEF*CCOEF/ABM  - 1/(MN3^2) )
            FPM = ABMC * ( (2.0 * bcoef * ccoef) / ABM - 1.0 / (MN3**2) )
    
            OMN3 = MN3
            MN3 = OMN3 - FM / FPM
    
            # --- Enforce the same “jump‐across‐1.0” logic as Fortran ---
            # If MN3 > 1.0 and OMN3 < 1.0, force MN3 = 0.99
            if MN3 > 1.0 and OMN3 < 1.0:
                MN3 = 0.99
    
            # If MN3 < 1.0 and OMN3 > 1.0, force MN3 = 1.01
            if MN3 < 1.0 and OMN3 > 1.0:
                MN3 = 1.01
    
            # --- The Fortran “IF (MN3,GE,000) GO TO 80” is equivalent to “if MN3 >= 0.0” ---
            if MN3 < 0.0:
                # If MN3 turned negative, flip its sign and continue to next iteration
                MN3 = -MN3
                continue
    
            # Label 80: check for convergence:  IF (ABS(MN3‐OMN3)/OMN3 <= 0.0005) GO TO 100
            # In Python, we simply break out of the loop if converged:
            if abs(MN3 - OMN3) / OMN3 <= 0.0005:
                break
        else:
            # If we get here, we didn't break out of the loop
            print(f"***** THE 1-D SOLUTION FOR THE INITIAL-DATA SURFACE FAILED TO CONVERGE IN 20 ITERATIONS AT L={l} *****")
    
            # Otherwise, loop goes back (to the next ITER)
        # Label 100: MN3 is either converged or the last iterate
        return MN3

    for l in range(control.lmax): 
    
        x = geometry.xi + geometry.dx * l
        
        if control.n1d < 0:
            if geometry.ndim == 1:
                rads = geometry.yw[l]**2 - gcb.ycb[l]**2
                aratio = rads / control.rstars
                mn3 = newton_raphson_mach()
            else:
                rad = geometry.yw[l] - gcb.ycb[l]
            ...
            if nxck == 1:
                ...
                if control.n1d == 1 or control.n1d == 3: mn3 = 1.1
                if control.n1d == 2 or control.n1d == 4: mn3 = 0.9
                nxck = 1
                if geometry.ndim == 1:
                    rads = geometry.yw[l]**2 - gcb.ycb[l]**2
                    aratio = rads / control.rstars
        ...
        dnxny = (geometry.nxny[l] - gcb.nxnycb[l]) / control.m1
 
        # Loop over M index (equivalent to DO 120 M=1,MMAX)
        for m in range(control.mmax):
            ...
            if dns == 0.0:
                ...
                if dn > 0.0:
                    ...
                    solution.v[l, m, 0] = sign * q / np.sqrt(1.0 + 1.0 / dns)            
    
    return solution



def onedim_old(control, geometry, gcb, bcc, solution):
    # Initialize variables
    mn3    = 2.0 if control.n1d == -1 or control.n1d > 2 else 0.01  # Set mach number guess to subsonic or supersonic
    grgas  = 1.0 / (control.rgas * control.g)
    nxck   = 0
    ...
    for l in range(control.lmax): 
        if l == 0 and (control.n1d == -1 or control.n1d > 2): continue
        ...
        if control.n1d >= 0:
            if gcb.ngcb != 0:
                if l == geometry.lt:
                    mn3 = 1.0
                elif l > geometry.lt and nxck == 0:
                    if control.n1d == 1 or control.n1d == 3:
                        mn3 = 1.1
                    if control.n1d == 2 or control.n1d == 4:
                        mn3 = 0.9
                    nxck = 1
            else:
                if x == geometry.xt:
                    mn3 = 1.0
                elif x > geometry.xt and nxck == 0:
                    if control.n1d == 1 or control.n1d == 3:
                        mn3 = 1.1
                    if control.n1d == 2 or control.n1d == 4:
                        mn3 = 0.9
        ...
        for iter_count in range(20):
            ...
            # Check for crossover between subsonic and supersonic
            if mn3 > 1.0 and omn3 < 1.0:
                mn3 = 0.99
            if mn3 < 1.0 and omn3 > 1.0:
                mn3 = 1.01
...
def create_test_data():
    ...
    control.lmax    = 80                # [-] (int) Number of points in axial direction
