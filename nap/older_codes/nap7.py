...
class Solution:
    def __init__(self, l_max=80, m_max=5, n_max=400, x_i=0.007874, r_i=0.0635, r_t=0.02032, x_e=0.1143, rc_i=0.02032, rc_t=0.0127, 
        ...
        self.T          = np.zeros((1, self.m_max, self.l_max))         # [K] Temperature Field
    ...
    def one_dimisional(self):
        ...
        def Newton_Raphson(M_guess, A_ratio):
            #Useful Parameters
            ...
            f = Mach_f(M_guess, self.gamma, A_ratio)
            df = Mach_df(M_guess, self.gamma)
            M = np.zeros(MaxIter+1)    
            M[0] = M_guess - (f/df)
            
            # Iterate using Newton-Raphson to determine the actual mach number    
            while (M[j-1] >=tol) and (j<=MaxIter):
                f = Mach_f(M[j-1], self.gamma, A_ratio)
                df = Mach_df(M[j-1], self.gamma)
                M[j] = M[j-1] - (f/df)
                j = j+1
        ...
        # Calculate P, RO, U, and V for the initial surface from the local Mach number             
            for M in range(0, self.m_max):
                ...
                self.T[0][M][L] = self.P[0][M][L] / (self.RO[0][M][L] * self.R_gas);
    ...
    def masflo(self):
        ...
        mass_i      = self.RO[0, 0, 0] * vm_i * a_inlet; print(mass_i, a_inlet)
        ...
        print(f'Expansion Ratio = {self.a_e / self.a_t:<3.2f} [-], vel_exit = {vm_e:<3.2f} [m/s], mdot = {cv.convert(mass_i, "kg/s", "g/s"):<3.5f} [g/s], thrust = {cv.convert(thrust, "N", "mN"):<3.2f} [mN]')



    def print_one_dim_data_base_units(self):
        cols = {"L [-]": 5, "x [m]": 8, "U [m/s]": 10, "V [m/s]": 10, "P [Pa]": 12, "RO [kg/m3]": 12, "Mach [-]": 10, "T [degK]": 10}
        ...
        for idx in range(0, self.l_max):
            print(f"{idx:>{cols['L [-]']}}|{self.xw[idx]:{cols['x [m]']}.2f}|"
                  ...
                  f"{self.M_one_dim[idx]:>{cols['Mach [-]']-2}.2f}|{self.T[0, 0, idx]:>{cols['T [degK]']}.2f}")

    def print_one_dim_data_mixed_units(self):
        cols = {"L [-]": 5, "x [m]": 8, "U [m/s]": 10, "V [m/s]": 10, "P [Pa]": 12, "P [psi]": 8, "RO [kg/m3]": 12, "Mach [-]": 10, "T [degK]": 10}
    ...
    def print_one_dim_data_eng_units(self):
        ...
        for idx in range(0, self.l_max):
            print(f"{idx:>{cols['L [-]']}}|{cv.convert(self.xw[idx], 'm', 'in'):{cols['x [in]']}.2f}|"
                  ...
                  f"{self.M_one_dim[idx]:>{cols['Mach [-]']-2}.2f}|{cv.convert(self.T[0, 0, idx], 'K', 'degF'):>{cols['T [degF]']}.2f}")
...
if __name__ == '__main__':
    ...
    # 100 mN ASCENT thruster
    c1 = {'x_i':  cv.convert(.15, 'in', 'm'),   'r_i':  cv.convert(3, 'mm', 'm'),
          'r_t':  cv.convert(0.29, 'mm', 'm'),   'x_e':  cv.convert(21, 'mm', 'm'),
          'rc_i': cv.convert(7, 'mm', 'm'),    'rc_t': cv.convert(2, 'mm', 'm'),
          'PT':   cv.convert(379, 'psi', 'Pa'),   'TT':   cv.convert(1880, 'degC', 'K'),
          'PE':   cv.convert(0, 'psi', 'Pa'), 'THETA': 0.0,
          'R_gas': cv.convert(53.353, 'ft*lbf/lb*degR', 'J/kg*K'), 'gamma': 1.21, 
          'FDT': 1.6, 'ang_i': 44.88, 'ang_e': 15.0, 't_conv': 3.0e5, 't_stop': 1.0}
    ...
    a.print_one_dim_data_base_units()
